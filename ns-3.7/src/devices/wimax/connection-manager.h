...
public:
   ...
   */
  Ptr<WimaxConnection> CreateConnection (Cid::TypeCid type);
  /**
   * \brief add a connection to the list of managed connections
   * \param connection the connection to add
   * \param type the type of connection to add
   */
  void AddConnection (Ptr<WimaxConnection> connection, Cid::TypeCid type);
  // Added by Ramon
   ...
   */
  std::vector<Ptr<WimaxConnection> > GetConnections (Cid::TypeCid type) const;
  uint32_t GetNPackets (Cid::TypeCid type, ServiceFlow::SchedulingType schedulingType) const;
  /**
