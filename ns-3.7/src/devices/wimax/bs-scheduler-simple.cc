                                          ...
                                          Ptr<PacketBurst> burst)
{
  ...
  if (connection->GetType () == Cid::CID_TRANSPORT)
    {
      ...
      if (connection != GetBs ()->GetInitialRangingConnection () && connection != GetBs ()->GetBroadcastConnection ())
        {
          ...
          if (connection->GetType () == Cid::CID_MULTICAST)
            {
//               modulationType = connection->GetServiceFlow ()->GetModulation ();
      ...
      if (connection->GetType () == Cid::CID_TRANSPORT || connection->GetType () == Cid::CID_MULTICAST)
        {
  ...
  if (m_downlinkBursts->size ())
    {
      NS_LOG_DEBUG ("BS scheduler, number of bursts: " << m_downlinkBursts->size () << ", symbols left: "
                                                       ...
                                                       << GetBs ()->GetConnectionManager ()->GetNPackets (Cid::CID_BASIC, ServiceFlow::SF_TYPE_NONE) << " primary "
                                                       << GetBs ()->GetConnectionManager ()->GetNPackets (Cid::CID_PRIMARY, ServiceFlow::SF_TYPE_NONE) << " transport "
                                                       << GetBs ()->GetConnectionManager ()->GetNPackets (Cid::CID_TRANSPORT, ServiceFlow::SF_TYPE_ALL));
    }
}
...
bool BSSchedulerSimple::SelectConnection (Ptr<WimaxConnection> &connection)
{
  ...
  if (GetBs ()->GetBroadcastConnection ()->HasPackets ())
  ...
  else
    {
      ...
      // Added by Ramon
      connections = GetBs ()->GetConnectionManager ()->GetConnections (Cid::CID_MULTICAST);
      for (iter1 = connections.begin (); iter1 != connections.end (); ++iter1)
        {
          if ((*iter1)->HasPackets ())
            {
              NS_LOG_INFO ("Return Multicast");
              connection = *iter1;
              return true;
            }
        }

      connections = GetBs ()->GetConnectionManager ()->GetConnections (Cid::CID_BASIC);
      for (iter1 = connections.begin (); iter1 != connections.end (); ++iter1)
        {
          if ((*iter1)->HasPackets ())
            {
      ...
      connections = GetBs ()->GetConnectionManager ()->GetConnections (Cid::CID_PRIMARY);
      for (iter1 = connections.begin (); iter1 != connections.end (); ++iter1)
        {
          if ((*iter1)->HasPackets ())
            {
