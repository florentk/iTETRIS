...
//   std::cout << "[WimaxNetDevice::Send] to=" << to << " dest="<< dest << " my=" << Mac48Address::ConvertFrom (GetAddress ()) <<" proto="<< protocolNumber << std::endl;
  m_traceTx (packet, to);
...
{
//   std::cout << "[WimaxNetDevice::ForwardUp] Successful reception - Commented" << std::endl;
  m_traceRx (packet, source);
  LlcSnapHeader llc;
  packet->RemoveHeader (llc);
//   Ptr<Node> node = GetNode ();
//   Ptr<MobilityModel> mob = node->GetObject<MobilityModel> ();
//   std::cout << "[WimaxNetDevice::ForwardUp] Distance=" << mob->GetPosition().x <<std::endl;
...
{
  m_initialRangingConnection = CreateObject<WimaxConnection> (Cid::InitialRanging (), Cid::CID_INITIALRANGING);
  m_broadcastConnection = CreateObject<WimaxConnection> (Cid::Broadcast (), Cid::CID_BROADCAST);
}
...
//   std::cout << "WimaxNetDevice::Receive, station = " << GetMacAddress () << std::endl;
...
WimaxNetDevice::ForwardDown (Ptr<PacketBurst> burst, WimaxPhy::ModulationType modulationType)
{
  ...
  m_phy->Send (params);
//   std::cout << "WimaxNetDevice::ForwardDown" << std::endl;
  delete params;
}
