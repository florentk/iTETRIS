  ...
  if (m_downlinkBursts->size ())
    {
      NS_LOG_DEBUG ("BS scheduler, number of bursts: " << m_downlinkBursts->size () << ", symbols left: "
                                                       ...
                                                       << " basic "
                                                       << GetBs ()->GetConnectionManager ()->GetNPackets (Cid::CID_BASIC, ServiceFlow::SF_TYPE_NONE)
                                                       << " primary "
                                                       << GetBs ()->GetConnectionManager ()->GetNPackets (Cid::CID_PRIMARY, ServiceFlow::SF_TYPE_NONE)
                                                       << " transport "
      ...
      if ((*iter2)->HasPackets ())
        {
          ...
          if (rtPSConnection[nbConnection]->GetType () == Cid::CID_MULTICAST)
            {
